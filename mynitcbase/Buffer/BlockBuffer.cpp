...
int BlockBuffer::getHeader(struct HeadInfo *head)
{
    ...
    Disk::readBlock(buffer, this->blockNum);    // read the block at this.blockNum into the buffer

    memset(head, 0, sizeof(struct HeadInfo));   // populate the numEntries, numAttrs and numSlots fields in *head
...
int RecBuffer::getRecord(union Attribute *rec, int slotNum)
{
    ...
    */
    ...
    memset(rec, 0, recordSize);     // load the record into the rec data structure
